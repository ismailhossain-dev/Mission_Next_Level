//localhost:5000/api/user ei get korle eta hit hobe
...
const auth = (...roles: ROLES[]) => {
  return async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      ) as JwtPayload; //jwtPlaylod use nakore docoded.email use korle type error dive
     ...
     */
      ...
      if (roles.length && !roles.includes(user.role)) {
        return res.status(403).json({
          success: false,
          message: "Forbidden!! | This route have no access! ",
        });
      }
